...
const dartaArray = [
	{ratingRevievs: "264 отзыва", price: {oldUan: "4 333 грн", newUan: "3799 грн"}, name: "Motorola MOTO G4 (XT1622) Black"}, 
	...
	{ratingRevievs: "86 отзывов", price: {oldUan: "24 999 грн", newUan: "22999 грн"}, name: "Samsung Galaxy S8 64GB Midnight Black + карта памяти 64гб + оригинальный чехол!"}, 
	...
	{ratingRevievs: "14 отзывов", price: "11 499 грн", name: "Apple iPhone 632GB Space Gray"}, 
	{ratingRevievs: "70 отзывов", price: {oldUan: "7399 грн", newUan: "5 999 грн"}, name: "Asus ZenFone 2 32GB (ZE551ML) Black"}, 
...
function sortByFeedbacks(arr) {
	const temp = JSON.parse(JSON.stringify(arr));		//crete deep copy of array, without references to parent array
	
	temp.forEach(item => {
		item.ratingRevievs = +item.ratingRevievs.replace(/\D/g, '');  //RegExp: all not numbers replace vs empty '';  + modify to numbers
	});

	temp.sort((a, b) => a.ratingRevievs > b.ratingRevievs ? 1 : -1);  //sorting from lovest to highest
	
	document.querySelector('.result').innerHTML = '';					//clear previous sorting results

	temp.forEach(item => {
        if (typeof(item.price) === 'string') {							//check if price within object
...
function sortByPrice(arr) {
    ...
    temp.sort((a, b) => a.price > b.price ? 1 : -1);  //sorting from lovest to highest
